...
**Estado:** Aprobado — 27/07/2026. Ejecutado vía FC-04 (encabezado `List-Unsubscribe`, no requiere remitente de Google como se pensaba inicialmente): el correo sintético se autoenvió mediante un script temporal de una sola vez usando el servicio avanzado de Gmail (`Gmail.Users.Messages.send()` con MIME crudo), ya que la ventana de redactar normal de Gmail no permite fijar ese encabezado. `Log Mensajes` confirmó `estado=SIN_TAREAS`, `etapa=FINALIZADO`, 0 observaciones, 0 tareas, `resultado_gmail=SOLO_ETIQUETADO`, sin `modelo` (cero llamadas a OpenAI), con la columna `error` mostrando "Encabezado List-Unsubscribe presente (boletín, promoción o comunicación masiva)."; `Registro Tareas` sin fila nueva; `Indice Idempotencia` con una entrada (`task_id` vacío, `estado_final=SIN_TAREAS`); Gmail con la etiqueta `Revisión manual/Sin tareas detectadas` aplicada, sin `Procesado`. Aprobó al primer intento real (un log previo mostró una línea de instrumentación de CP-29 ya retirada del repo, por una copia desactualizada de `script_refactorizado.gs` en el proyecto de prueba — no afectó este resultado). Ver detalle completo en `pruebas/resultados/RESULTADOS_FASE_8.md`.
...
**Estado:** Aprobado — 26/07/2026. Ejecutado mediante instrumentación temporal gateada (`cfg.modoPrueba` + property exclusiva `CP08_FORZAR_JSON_INVALIDO`) en `consultarIAExtractora()` (`codigo/cliente_openai.gs`, no `script_refactorizado.gs`): la función devolvió `contenidoCrudo: 'esto no es json'` sin llamar a la API real de OpenAI. Una sola corrida (sin `Logger.log('consultarIAExtractora(): usando prompt versión...')`, confirmando que no hubo consulta real): `Log Mensajes` quedó en `estado=REVISION_MANUAL`, `etapa=FINALIZADO`, sin fila en `Registro Tareas`, `Indice Idempotencia` con una entrada (`task_id` vacío), etiqueta de Gmail `Revisión manual/Error de procesamiento` aplicada, sin filas en ninguna hoja de negocio. Aprobó al primer intento. Instrumentación ya retirada del código. Ver detalle completo en `pruebas/resultados/RESULTADOS_FASE_8.md`.
...
**Estado:** Aprobado — 26/07/2026 (opción b). Ejecutado mediante instrumentación temporal gateada (`cfg.modoPrueba` + property exclusiva `CP09_FORZAR_HTTP_TEMPORAL`) en `consultarIAExtractora()` (`codigo/cliente_openai.gs`): se reemplazó solo el objeto `response` de `UrlFetchApp.fetch()` (HTTP 503 simulado en el intento 1, HTTP 200 con contenido válido en el intento 2), conservando intacto el resto del bucle real de reintentos. Prueba de una sola corrida (el reintento es interno a la función). Correo sintético "Consulta sobre horario de atención". Log: ambos intentos simulados sin ninguna línea de error; `Log Mensajes.intentos=2`, `estado=PROCESADO`, `cantidad_observaciones=1`, `cantidad_tareas=1`, 1 fila nueva en `Registro Tareas` y en `Desarrollo IT`, etiqueta `Procesado` en Gmail. Dos intentos previos se descartaron por una copia desactualizada del archivo en el proyecto de prueba (le faltaba el campo `grupo_origen`, corregido tras un chequeo local antes de la primera corrida). Instrumentación ya retirada del código. Ver detalle completo en `pruebas/resultados/RESULTADOS_FASE_8.md`. **Advertencia de seguridad:** especialmente en la opción (a) (no usada aquí), nunca registrar el valor configurado de `OPENAI_API_KEY` ni `cfg` completo al depurar el HTTP 401; si hace falta inspeccionar la configuración, usar `serializarSeguro()`/`ejecutarValidacionVisible()` (`pruebas/debug_seguro_pruebas.gs`).
...
**Estado:** Aprobado — 26/07/2026. Ejecutado mediante instrumentación temporal gateada (`cfg.modoPrueba` + property exclusiva `CP25_FORZAR_FALLO_GMAIL`, mismo punto y mecanismo que CP-12 Variante A) en `aplicarResultadoGmail()`, ya retirada del código. Correo sintético "Reportes internos sin actualizar" (`message_id 19fa0743dc9d5b94`, 2 tareas: `Desarrollo IT`/revisar el servidor de reportes, `Comercial`/avisar a los clientes por la demora). Primera corrida: `Log Mensajes` quedó en `estado=ERROR_TEMPORAL`, `etapa=ESCRITURA_COMPLETADA`, sin entrada en `Indice Idempotencia`, con las 2 tareas `ESCRITA` en `Registro Tareas`. Segunda corrida (instrumentación desactivada, ejecutada de inmediato sin esperar ningún umbral de tiempo): `"1 mensajes elegibles, procesando 1"` → `"procesarUnMensaje(): existe manifiesto... se reanuda sin volver a consultar la IA"` → `"reanudarDesdeManifiesto(): todas las tareas... ya estaban ESCRITA; se repite únicamente la actualización de Gmail"`; `Log Mensajes` pasó a `PROCESADO`; confirmado visualmente que `Desarrollo IT`/`Comercial` mantienen exactamente 1 fila cada una, sin duplicar. A diferencia de CP-12 (ambas variantes), aprobó al primer intento sin ningún hallazgo adicional. Ver detalle completo en `pruebas/resultados/RESULTADOS_FASE_8.md`.
...
**Historial (hallazgo H-05/H-06, auditoría 20/07/2026):** este caso estuvo bloqueado como prueba obligatoria hasta que `finalizarMensaje()` implementara upsert por `message_id`+`task_id` (`documentacion/RECUPERACION_INTERRUPCIONES.md`, sección 9) — con el código anterior, el caso solo podía confirmar que la duplicación "no ocurrió esta vez", no que estuviera estructuralmente prevenida.

**Procedimiento:** forzar dos invocaciones reales de `finalizarMensaje()` para el mismo mensaje/tareas (instrumentación temporal gateada por `cfg.modoPrueba` + `CP35_DUPLICAR_FINALIZACION`, ya retirada del código) y confirmar una sola fila final por `task_id` en `Indice Idempotencia`.
**Resultado esperado:** ninguna fila duplicada con la misma combinación `message_id` + `task_id`; el valor final refleja genuinamente la segunda invocación (prueba de que es una actualización real, no un no-op ni una fila fantasma).
**Estado:** Aprobado — 27/07/2026. Corrección aplicada en `codigo/script_refactorizado.gs` (nueva `upsertIndiceIdempotencia()`, indexada por la clave compuesta `message_id+task_id`; `finalizarMensaje()` reordenado para confirmar ese upsert antes de `actualizarLogMensajes()`, cerrando también H-06). Verificado localmente con mocks de Sheets (20 verificaciones: inserción nueva, doble invocación mismo estado, doble invocación distinto estado, lote mixto, orden de llamadas, y ausencia de recursión sin límite en la instrumentación). Corrida real (`message_id 19fa1dc793d189d7`, `task_id ALI-373E343149F446E3-001`): la línea de instrumentación apareció exactamente una vez (sin recursión descontrolada); `Indice Idempotencia` mostró una sola fila para esa combinación con `estado_final=CP35_SEGUNDA_LLAMADA`; `Log Mensajes` mostró la misma fila (no duplicada) con `estado=CP35_SEGUNDA_LLAMADA`/`etapa=FINALIZADO`. Aprobó al primer intento real. Instrumentación temporal retirada. **Con esto queda cerrado el último punto que condicionaba la aprobación de la Fase 8 (H-05/H-06).** Ver detalle completo en `pruebas/resultados/RESULTADOS_FASE_8.md`.
...
**Estado:** Aprobado — 26/07/2026. Ejecutado mediante instrumentación temporal gateada (`cfg.modoPrueba` + property exclusiva `CP26_FORZAR_FALLO_ESCRITURA`) en `procesarUnMensaje()`, justo después de `persistirManifiestoTareas()`/`ETAPA.TAREAS_RESERVADAS` y antes de `escribirFilasPorLote()`, ya retirada del código. Correo sintético "Certificado de seguridad por vencer" (`message_id 19fa0a67abbf10f3`, 2 tareas: `Desarrollo IT`/renovar el certificado, `Comercial`/informar la renovación). Primera corrida: `Log Mensajes` quedó en `estado=ERROR_TEMPORAL`, **`etapa=TAREAS_RESERVADAS`** (a diferencia de CP-12/CP-25, que llegan a `ESCRITURA_COMPLETADA`), con las 2 tareas `RESERVADA` en `Registro Tareas` (`task_id` asignado, sin `fila_destino`); sin filas en `Desarrollo IT`/`Comercial`. Segunda corrida (instrumentación desactivada, ejecutada de inmediato): `"procesarUnMensaje(): existe manifiesto... se reanuda sin volver a consultar la IA"`, sin ninguna línea `consultarIAExtractora()`; `Log Mensajes` pasó a `PROCESADO`; confirmado que `Desarrollo IT`/`Comercial` recibieron 1 fila nueva cada una, con los **mismos `task_id`** ya reservados en la primera corrida — sin generar un manifiesto nuevo. Un primer intento con un correo distinto se descartó sin usarse (la property no se había llegado a crear en Script Properties; el mensaje se procesó normalmente de punta a punta y quedó cerrado). Ver detalle completo en `pruebas/resultados/RESULTADOS_FASE_8.md`. **Advertencia de seguridad:** la instrumentación de este caso no debe loguear `cfg` ni `options` en ningún punto de la interrupción forzada; usar `serializarSeguro()` si se necesita ver el estado del manifiesto u otro objeto no sensible. (Cumplida: el mensaje de la excepción solo incluye el `messageId`.)
...
**Estado:** Aprobado — 27/07/2026. Primera corrida real reveló un dato sensible sin enmascarar (tarjeta, por un separador NBSP que el patrón no reconocía — INC-FASE8-012); corrección aplicada en `codigo/prompts_ia.gs` y segunda corrida real (mensaje nuevo) confirmó ambos valores enmascarados correctamente (`[TARJETA_ENMASCARADA]` y `[DNI_ENMASCARADO]`), con el procesamiento continuando con normalidad. Instrumentación temporal retirada. Ver detalle completo en `pruebas/resultados/RESULTADOS_FASE_8.md` e INC-FASE8-012 (cerrada) en `pruebas/resultados/INCIDENCIAS_FASE_8.md`. **Endurecimiento adicional (decisión post-aprobación):** DNI y CBU también se endurecieron para tolerar separadores de espacio en blanco (incluido NBSP); ver `auditoria/CHANGELOG.md` para el detalle, incluido un riesgo residual documentado (falso positivo con listas numeradas largas, preexistente desde la Fase 4, no corregido).
...
**Procedimiento:** forzar una falla después de que el mensaje ya fue archivado por una llamada previa exitosa (instrumentación temporal gateada por `cfg.modoPrueba` + `CP38_FORZAR_FALLO_POSTERIOR` en `procesarUnMensaje()`, con `PERMITIR_ARCHIVADO=true` temporal — ya retirada del código), de modo que el mensaje quedara `ERROR_TEMPORAL` con manifiesto pero fuera de `in:inbox`.
**Resultado esperado:** en la ejecución siguiente, el mensaje se recupera igual, sin depender de que la búsqueda de Gmail lo encuentre.
**Estado:** Aprobado — 27/07/2026. Corrida real (`message_id 19fa40fc2e504081`): la primera ejecución instrumentada archivó el mensaje de verdad y luego forzó la falla, dejándolo `ERROR_TEMPORAL` con manifiesto y fuera de `in:inbox`. La ejecución siguiente registró `"recuperarMensajesConManifiestoPendiente(): 19fa40fc2e504081 en ERROR_TEMPORAL con manifiesto persistido; reanudando sin depender de la búsqueda de Gmail"`, mientras que la búsqueda normal (`procesarCorreosDeTareas()`) informó `"0 mensajes elegibles, procesando 0"` — confirmando que la recuperación no dependió en absoluto de que Gmail volviera a encontrar el mensaje, sino exclusivamente de la nueva función. `reanudarDesdeManifiesto()` detectó las tareas ya `ESCRITA` y repitió únicamente la actualización de Gmail. Verificado en la planilla real: `Log Mensajes` con `estado=PROCESADO`, `etapa=FINALIZADO`, `error` vacío (H-12) y `unidades_gmail_api=2` (acumulado, H-11); `Indice Idempotencia` con las 2 filas nuevas (`estado_final=PROCESADO`); `Registro Tareas` sin duplicar (las mismas 2 filas `ESCRITA` de la corrida original). Aprobó al primer intento real. Instrumentación temporal retirada. Ver detalle completo en `pruebas/resultados/RESULTADOS_FASE_8.md`.
...
**Hallazgo H-14 (detectado al preparar este caso, antes de instrumentar o correr nada):** con H-07 ya activo, un mensaje `ERROR_TEMPORAL` con manifiesto que no se archiva (el escenario exacto de este caso) se encontraba dos veces por ejecución — vía `recuperarMensajesConManifiestoPendiente()` y vía el chequeo de manifiesto de `procesarUnMensaje()` — duplicando `intentos_gmail` por corrida. Corregido y verificado localmente (DEC-012, `documentacion/RECUPERACION_INTERRUPCIONES.md`, sección 14) antes de correr este caso.
...
**Resultado esperado:** tras agotar el límite, el mensaje se cierra `ERROR_DEFINITIVO`, se escribe `Indice Idempotencia` (deja de reintentarse), y las tareas ya escritas en los tableros de negocio **permanecen** (no se revierten). Antes de agotar el límite, el mensaje debe seguir reintentándose normalmente en cada ejecución, exactamente 1 vez por ejecución (confirma H-14).
**Estado:** Aprobado — 27/07/2026. Corrida real (`message_id 19fa443c94a40af2`, `LIMITE_REINTENTOS_GMAIL=6` en el proyecto de prueba): 7 ejecuciones manuales sucesivas. La primera generó y escribió 2 tareas y falló al actualizar Gmail (`intentos_gmail: 0→1`); las siguientes 5 fueron recuperadas exclusivamente por `recuperarMensajesConManifiestoPendiente()` (H-07) — cada una confirmó `"0 mensajes elegibles, procesando 0"` en la búsqueda normal, probando que H-14 excluye correctamente al mensaje de un segundo intento en la misma ejecución (`intentos_gmail` avanzó de a 1 por corrida: 2, 3, 4, 5, 6). La séptima superó el límite y cerró `ERROR_DEFINITIVO` (`"superó LIMITE_REINTENTOS_GMAIL (6); cierre ERROR_DEFINITIVO con las tareas ya escritas conservadas"`). Verificado en la planilla real: `Log Mensajes` con `estado=ERROR_DEFINITIVO`, `etapa=FINALIZADO`, `error` con el texto de la instrumentación **conservado** (confirma H-12: solo se limpia en `PROCESADO`, no en otros cierres), `intentos_gmail=7`; `Indice Idempotencia` con las 2 filas nuevas (`estado_final=ERROR_DEFINITIVO`); `Registro Tareas` sin duplicar (las mismas 2 filas `ESCRITA` originales). Nota: `resultado_gmail`/`unidades_gmail_api` no se modificaron durante esta prueba — el gancho de instrumentación interrumpe `aplicarResultadoGmail()` antes de la llamada real a `Gmail.Users.Messages.modify()`, así que nunca hubo una llamada real a la API en ninguna de las 7 corridas (comportamiento esperado de este mecanismo específico, no un defecto). Instrumentación temporal retirada. Ver detalle completo en `pruebas/resultados/RESULTADOS_FASE_8.md`.
...
| CP-38 | `recuperarMensajesConManifiestoPendiente()` (H-07, DEC-010) aplicada y confirmada con corrida real (27/07/2026) — sin dependencias pendientes |
| CP-39 | `LIMITE_REINTENTOS_GMAIL`/`intentos_gmail` (H-08, DEC-007) aplicada y confirmada con corrida real (27/07/2026) — sin dependencias pendientes |
...
**CP-38 y CP-39** quedaron Aprobados el 27/07/2026 (Lotes 2/3 — H-07, H-08 — con corrida real confirmada para ambos; ver detalle arriba). Con esto, **los Lotes 2 y 3 de la auditoría del 20/07/2026 (H-07, H-08, H-10, H-11, H-12, y el hallazgo adicional H-14 detectado y corregido al preparar CP-39) quedan completamente cerrados**: decididos, aplicados y confirmados con evidencia real. El hallazgo H-05/H-06 (idempotencia estructural de `finalizarMensaje()`) quedó resuelto y confirmado con CP-35 (Aprobado, 27/07/2026).
