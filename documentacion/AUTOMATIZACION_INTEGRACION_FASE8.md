...
| `INT-FASE8-09-FECHA-LIMITE-NO-EXPLICITA` | CP-18 | Equivalente a PE-05 (`pruebas/PRUEBAS_ESCRITURA.md`): complemento exacto de CP-17 — una tarea sin ninguna fecha mencionada en el cuerpo: `PROCESADO`, 1 observación, 1 tarea (`Desarrollo IT`), columna "Fecha límite" vacía. Reutiliza `efectoFormalUnaTareaConFechaFabrica_` sin ningún cambio de código (ya generalizada por CP-17). **CP-18 Aprobado (24/07/2026)**, al primer intento, sin necesitar ajuste (sección 9.7.1). |
| `INT-FASE8-10-ERROR-AUTOMATIZACION-APPS-SCRIPT` | CP-07 | Equivalente a FC-01 (`pruebas/CASOS_CORREOS_NO_OPERATIVOS.md`): asunto que coincide con la regla obligatoria de notificaciones de fallos de Apps Script (regla 1 de `evaluarFiltroDeterministico()`) — el asunto, no el remitente, dispara la regla (condición OR), permitiendo una corrida real desde `sichar@gmail.com` sin spoofing. Mismo mecanismo de filtro determinístico que CP-16 (sin llamada a OpenAI), pero con etiqueta `RevisionErrorAutomatizacion` en vez de `RevisionSinTareas`. **CP-07 Aprobado (24/07/2026)**, al primer intento, sin necesitar ajuste, con confirmación visual directa de la etiqueta en Gmail (sección 9.8.1). |

La propiedad `AUTO_FASE8_CASO` (`ScriptProperties` del proyecto de prueba) selecciona el fixture activo por `id`; si está ausente, se usa `FIXTURE_INTEGRACION_POR_DEFECTO` (`INT-FASE8-01-INFORMATIVO`). El automatizador **solo lee** esta propiedad — nunca la escribe ni modifica ninguna otra propiedad de configuración productiva. Para ejecutar el caso de CP-03, configurar `AUTO_FASE8_CASO=INT-FASE8-02-DOS-TAREAS`; para CP-04, `AUTO_FASE8_CASO=INT-FASE8-04-TRES-TAREAS`; para CP-16, `AUTO_FASE8_CASO=INT-FASE8-07-CUERPO-VACIO`; para CP-17, `AUTO_FASE8_CASO=INT-FASE8-08-FECHA-LIMITE-EXPLICITA`; para CP-18, `AUTO_FASE8_CASO=INT-FASE8-09-FECHA-LIMITE-NO-EXPLICITA`; para CP-07, `AUTO_FASE8_CASO=INT-FASE8-10-ERROR-AUTOMATIZACION-APPS-SCRIPT`; en todos los casos, antes de llamar a `prepararCasoIntegracionFase8Visible()` (ver procedimiento, sección 9).
...
### 7.3. Verificación de la columna "Fecha límite"

**Ampliación (24/07/2026, CP-17/`INT-FASE8-08-FECHA-LIMITE-EXPLICITA`):** hasta esta ampliación, la verificación de hojas de negocio (sección 7) solo comprobaba la columna `ID` (para vincular las filas nuevas al manifiesto) — ninguna otra columna, incluida "Fecha límite" (`documentacion/MAPA_COLUMNAS.md`, fila 12), se comparaba nunca contra un valor esperado.

`verificarResultadoFormal_()` gana una verificación **opcional**, activada por `fixture.esperado.fechaLimiteEsperada` (string ISO `'YYYY-MM-DD'` o `null`), aplicable solo cuando hay exactamente 1 tarea esperada para el tablero en cuestión:

- Si `fechaLimiteEsperada` es una fecha: se exige que la celda sea un objeto `Date` cuyos componentes locales (año/mes/día, extraídos con `getFullYear()`/`getMonth()`/`getDate()` — nunca los equivalentes UTC ni `Utilities.formatDate()`) coincidan exactamente con esa fecha.
- Si `fechaLimiteEsperada` es `null`: se exige que la celda esté vacía (`''`) — ni `0`, ni la cadena `"null"`, ni una fecha por defecto.

Categoría cerrada: `HOJA_NEGOCIO_FECHA_LIMITE_NO_COINCIDE:<tablero>:<obtenido>` (donde `<obtenido>` es la fecha ISO local extraída, `VACIO` o `NO_ES_FECHA` — nunca el valor crudo de la celda). Ausente `fechaLimiteEsperada` (los siete fixtures anteriores), el comportamiento es idéntico al previo a esta ampliación.

Esta verificación existe para confirmar, en producción real, que `construirFechaLocal()` (`codigo/escritura_sheets.gs`) evita el corrimiento de un día documentado en `documentacion/MAPA_ESCRITURA.md`, sección 2 — construyendo la fecha con componentes explícitos (`new Date(año, mes-1, día)`) en vez de parsear el ISO 8601 directamente, lo que en Apps Script se interpreta en la zona horaria configurada del proyecto.
...
**Por qué una fecha explícita y no una relativa:** el prompt real (`codigo/prompts_ia.gs`) trae un ejemplo few-shot donde una referencia relativa de día ("antes del viernes") se clasifica con `fecha_limite: null` — una referencia relativa no cuenta como fecha explícita en formato calendario. Por eso este cuerpo usa una fecha concreta (31/07/2026, 7 días después de la fecha de esta redacción, para evitar cualquier ambigüedad de "hoy"), y no una redacción relativa como la del enunciado original de CP-17/CASOS_DE_PRUEBA.md.

**Qué verifica esta corrida (nuevo, sección 7.3):** que la columna "Fecha límite" de la hoja `Comercial` muestre exactamente `31/07/2026` (visualmente `dd/MM/yyyy`, verificado internamente por componentes de fecha ISO local `2026-07-31`) — confirmando que `construirFechaLocal()` no produce el corrimiento de un día documentado en `documentacion/MAPA_ESCRITURA.md`, sección 2. Esta es la primera corrida real de este automatizador que verifica el contenido de una columna de negocio más allá de `ID`.
...
### 9.7. Procedimiento para CP-18 (`INT-FASE8-09-FECHA-LIMITE-NO-EXPLICITA`)

**CP-18 Aprobado — 24/07/2026** (ver sección 9.7.1). El procedimiento que llevó a esa corrida (pasos 0-6 de la sección 9), con dos diferencias:

- Antes del paso 3 (`prepararCasoIntegracionFase8Visible()`), configurar en el proyecto de prueba la propiedad `AUTO_FASE8_CASO=INT-FASE8-09-FECHA-LIMITE-NO-EXPLICITA` (sección 6).
- El asunto/cuerpo a enviar (paso 4) serán los de este fixture: `[PRUEBA-AUTOMATIZACION][INTEGRACION] Actualización de la lista de precios [<marcador>]`, con el cuerpo: "Hay que actualizar la lista de precios que aparece en la sección de productos del sitio web."

**Complemento exacto de CP-17:** el cuerpo no menciona ninguna fecha, ni siquiera relativa. Verifica que la columna "Fecha límite" de la hoja `Desarrollo IT` quede **vacía** (no `0`, no la cadena `"null"`, no una fecha por defecto) — la rama opuesta de la verificación agregada en la sección 7.3, ya probada localmente (pruebas T4/T5 y U1) mediante una fábrica de doble reutilizada sin cambios de CP-17.

**Aserciones del piloto (INT-FASE8-09, equivalente a CP-18).** Dry-run: exactamente un mensaje elegible; cero cambios en Gmail y en las ocho hojas (técnicas y de negocio); clasificación esperada 1 observación / 1 tarea en `Desarrollo IT`. Formal: `Log Mensajes` con una fila, `estado=PROCESADO`, `etapa=FINALIZADO`, `cantidad_observaciones=1`, `cantidad_tareas=1`, `resultado_gmail=SOLO_ETIQUETADO`; `Registro Tareas` con exactamente 1 fila para el `message_id`; `Indice Idempotencia` con 1 entrada (`estado_final=PROCESADO`); una fila nueva en `Desarrollo IT` (vinculada por `ID`) cuya columna "Fecha límite" está vacía; Gmail conserva `Pruebas-Automatizacion` e `INBOX`, recibe `Procesado`, no recibe etiquetas de revisión/error, no se archiva.

Tras una corrida real satisfactoria, la aprobación de CP-18 (si corresponde) se registra por separado en `pruebas/CASOS_DE_PRUEBA.md`/`pruebas/resultados/RESULTADOS_FASE_8.md`, con el mismo criterio humano de revisión que el resto de los casos de prueba.

#### 9.7.1. Primer intento real (24/07/2026) — corrida completa exitosa, CP-18 Aprobado

```text
runId: 34ca060d-42b0-4175-95e7-fc7808532a2f
messageId: 19f96b3f0b156c2a (nuevo)
formal: FORMAL_OK
```

`consultarIAExtractora(): usando prompt versión v4-INC-FASE8-011-informativo-sin-tareas` confirmó que este fixture llegó a la IA; `ejecutarFormalYVerificarCasoIntegracionFase8Visible()` informó `[AUTO-FASE8] FORMAL_OK` para ese `runId`/`messageId`. `verificarResultadoFormal_()` confirmó automáticamente: `Log Mensajes` (`estado=PROCESADO`, `cantidad_observaciones=1`, `cantidad_tareas=1`, `resultado_gmail=SOLO_ETIQUETADO`); `Registro Tareas` con exactamente 1 fila (`task_id` no vacío, `estado_escritura=ESCRITA`, tablero `Desarrollo IT`); `Indice Idempotencia` con 1 entrada (`estado_final=PROCESADO`); una fila nueva en `Desarrollo IT` vinculada por `ID`, con la columna "Fecha límite" **vacía**; Gmail con `Procesado` aplicado, sin etiquetas de error/revisión, sin archivar.

**CP-18 pasa de Pendiente a Aprobado — 24/07/2026**, al primer intento, sin necesitar ningún ajuste de redacción. Junto con CP-17, confirma en producción real ambos lados de la verificación de la columna "Fecha límite" (sección 7.3). Detalle completo en `pruebas/CASOS_DE_PRUEBA.md` y `pruebas/resultados/RESULTADOS_FASE_8.md`.

### 9.8. Procedimiento para CP-07 (`INT-FASE8-10-ERROR-AUTOMATIZACION-APPS-SCRIPT`)

**CP-07 Aprobado — 24/07/2026** (ver sección 9.8.1). El procedimiento que llevó a esa corrida (pasos 0-6 de la sección 9), con dos diferencias:

- Antes del paso 3 (`prepararCasoIntegracionFase8Visible()`), configurar en el proyecto de prueba la propiedad `AUTO_FASE8_CASO=INT-FASE8-10-ERROR-AUTOMATIZACION-APPS-SCRIPT` (sección 6).
- El asunto/cuerpo a enviar (paso 4) serán los de este fixture: `[PRUEBA-AUTOMATIZACION][INTEGRACION] Summary of failures for Google Apps Script [<marcador>]`, con el cuerpo: "Your script "procesarCorreosDeTareas" has failed 3 times in the last 24 hours. Execution failed: Exception: Servicio de Gmail no disponible temporalmente."

**Por qué el asunto y no el remitente:** la regla 1 de `evaluarFiltroDeterministico()` (`codigo/filtros_correo.gs`) dispara con remitente `noreply-apps-scripts-notifications@google.com` **O** asunto que contiene `"Summary of failures for Google Apps Script"` — es una condición OR. El remitente exigido no es una dirección que el tester pueda enviar realmente (a diferencia de CP-06, diferido por este motivo — ver sección 11). Este fixture usa el asunto como disparador, sin necesitar ningún spoofing ni tooling adicional.

**Particularidad de este fixture (igual que CP-16):** el rechazo es determinístico (filtro), no una clasificación de la IA — esta corrida real tampoco debería generar ninguna llamada a la API de OpenAI. A diferencia de CP-16, la etiqueta de Gmail resultante es `RevisionErrorAutomatizacion` (no `RevisionSinTareas`).

**Aserciones del piloto (INT-FASE8-10, equivalente a CP-07).** Dry-run: exactamente un mensaje elegible; cero cambios en Gmail y en las ocho hojas (técnicas y de negocio); clasificación esperada 0 observaciones / 0 tareas (`SIN_TAREAS`/`NO_ELEGIBLE`). Formal: `Log Mensajes` con una fila, `estado=SIN_TAREAS`, `etapa=FINALIZADO`, `cantidad_observaciones`/`cantidad_tareas` en blanco, `resultado_gmail=SOLO_ETIQUETADO`; `Registro Tareas` sin ninguna fila nueva; `Indice Idempotencia` con 1 entrada (`estado_final=SIN_TAREAS`, `task_id` vacío); ninguna hoja de negocio recibe filas nuevas; Gmail conserva `Pruebas-Automatizacion` e `INBOX`, recibe `Revisión manual/Error de automatización`, no recibe `Procesado` ni las otras etiquetas de revisión/error, no se archiva.

Tras una corrida real satisfactoria, la aprobación de CP-07 (si corresponde) se registra por separado en `pruebas/CASOS_DE_PRUEBA.md`/`pruebas/resultados/RESULTADOS_FASE_8.md`, con el mismo criterio humano de revisión que el resto de los casos de prueba.

#### 9.8.1. Primer intento real (24/07/2026) — corrida completa exitosa, CP-07 Aprobado

```text
runId: 9a2f73ca-684b-48e0-9fb9-fbd5ffb57382
messageId: 19f96cb239f5ec62 (nuevo)
formal: FORMAL_OK
```

`ejecutarFormalYVerificarCasoIntegracionFase8Visible()` informó `[AUTO-FASE8] FORMAL_OK` para ese `runId`/`messageId`. El log no mostró ninguna línea `consultarIAExtractora()` — confirma, igual que CP-16, que el filtro determinístico rechazó el mensaje antes de llegar a la IA, sin generar ningún costo de OpenAI. `verificarResultadoFormal_()` confirmó automáticamente: `Log Mensajes` (`estado=SIN_TAREAS`, `etapa=FINALIZADO`, `cantidad_observaciones`/`cantidad_tareas` en blanco, `resultado_gmail=SOLO_ETIQUETADO`); `Registro Tareas` sin ninguna fila nueva; `Indice Idempotencia` con 1 entrada (`estado_final=SIN_TAREAS`, `task_id` vacío); ninguna hoja de negocio modificada; Gmail sin `Procesado` ni las otras etiquetas de revisión/error, sin archivar.

**Confirmación visual directa:** el tester revisó Gmail y confirmó que el mensaje recibió la etiqueta `Revisión manual/Error de automatización` — exactamente la esperada, distinta de la de CP-16.

**CP-07 pasa de Pendiente a Aprobado — 24/07/2026**, al primer intento, sin necesitar ningún ajuste. Confirma en producción real que la regla obligatoria de notificaciones de fallos de Apps Script dispara correctamente por asunto y aplica la etiqueta distinta correspondiente. Detalle completo en `pruebas/CASOS_DE_PRUEBA.md` y `pruebas/resultados/RESULTADOS_FASE_8.md`.
...
La corrección del 24/07/2026 (primer hallazgo real de CP-16, `messageId 19f9661d038ea8de`, sección 7.2/9.5.1) agregó el campo `esperado.resultadoSimulado` al fixture `INT-FASE8-07-CUERPO-VACIO` y amplió `verificarClasificacionSimulada_()` para verificar, contra la categoría `NO_ELEGIBLE`, que `cantidadObservaciones`/`cantidadTareas` sean `null` (nunca `0`) — la forma real que devuelve `procesarUnMensajeSimulado()` cuando el filtro determinístico rechaza un mensaje antes de la IA. También corrigió el doble compartido `clasificacionSimuladaPorDefecto_()` (usado por todos los fixtures sin `clasificacionSimuladaOverride`), que derivaba un `SIN_TAREAS` genérico en vez de reproducir `NO_ELEGIBLE` — por lo que la prueba S1 aprobaba localmente sin haber podido detectar esta discrepancia antes de la corrida real. Nuevas pruebas (S2-S4): `verificarClasificacionSimulada_()` directamente, con `NO_ELEGIBLE`/`null`/`null` (aprueba), un resultado distinto al esperado (`SIMULACION_RESULTADO_NO_COINCIDE`) y cantidades numéricas en vez de `null` (rechaza). Ningún fixture anterior (los seis restantes, sin `resultadoSimulado`) cambia de comportamiento. Resultado de esa ejecución local: 158/158 verificaciones OK (las 155 anteriores sin cambios + 3 nuevas).

La ampliación del 24/07/2026 (CP-17/`INT-FASE8-08-FECHA-LIMITE-EXPLICITA`) evaluó primero CP-06 como siguiente caso, pero sus dos equivalentes (FC-04/FC-09) requieren un encabezado o remitente que no se puede producir con un correo normal desde `sichar@gmail.com` — se difirió CP-06 y se continuó con CP-17 (fecha límite explícita, reutiliza PE-04, `pruebas/PRUEBAS_ESCRITURA.md`). Agregó el fixture y `crearEstadoFechaLimiteExplicita_`/`prepararSimuladoFechaLimiteExplicita_`, con una fábrica de efecto formal dedicada `efectoFormalUnaTareaConFechaFabrica_` (NO reutiliza `efectoFormalUnaTareaFabrica_` de CP-14/CP-15: escribe una capacidad nueva, la columna "Fecha límite", que esas no necesitaban). Amplió `verificarResultadoFormal_()` (sección 7.3) con una verificación opcional de esa columna, activada por `fixture.esperado.fechaLimiteEsperada`, sin afectar ningún fixture existente. Nuevas pruebas (sección T): camino correcto (fecha `2026-07-31` en `Comercial`); día equivocado (corrimiento de un día, `2026-07-30`); celda vacía cuando se esperaba una fecha; y la rama `fechaLimiteEsperada=null` (mutación temporal del fixture, aprobación con celda vacía y rechazo con una fecha presente — cubre el camino que usará CP-18 más adelante, aunque ese fixture todavía no existe). Resultado de esa ejecución local: 163/163 verificaciones OK (las 158 anteriores sin cambios + 5 nuevas).

La ampliación del 24/07/2026 (CP-18/`INT-FASE8-09-FECHA-LIMITE-NO-EXPLICITA`) agregó el fixture complemento exacto de CP-17 (sin ninguna fecha en el cuerpo) y `crearEstadoFechaLimiteNoExplicita_`/`prepararSimuladoFechaLimiteNoExplicita_`, reutilizando **sin cambios** `efectoFormalUnaTareaConFechaFabrica_` (creada para CP-17) con un tablero distinto (`Desarrollo IT`) y `fechaLimite:''` — no hizo falta ninguna fábrica nueva ni ningún cambio en `verificarResultadoFormal_()`, porque la rama `fechaLimiteEsperada=null` ya se había agregado y probado durante la ampliación de CP-17 (pruebas T4/T5). Una prueba de camino correcto (sección U) confirma que este fixture también aprueba. Resultado de esa ejecución local: 164/164 verificaciones OK (las 163 anteriores sin cambios + 1 nueva).

La ampliación del 24/07/2026 (CP-07/`INT-FASE8-10-ERROR-AUTOMATIZACION-APPS-SCRIPT`) agregó el fixture equivalente a FC-01 (asunto que dispara la regla obligatoria de fallos de Apps Script) y una fábrica de efecto formal **nueva**, `efectoFormalErrorAutomatizacionCorrecto_` (no reutiliza `efectoFormalSinTareasCorrecto_`, el valor por defecto ya aprobado por CP-16/INT-FASE8-01: ambas escriben el mismo `estado_final=SIN_TAREAS`, pero difieren en la etiqueta de Gmail aplicada — `L_ERRAUTO` en vez de `L_SINTAREAS`). No hizo falta ningún cambio en `verificarResultadoFormal_()`/`verificarClasificacionSimulada_()`, ya que `RevisionErrorAutomatizacion` ya era una clave de etiqueta soportada de forma genérica. Nuevas pruebas (sección V): camino correcto (`RevisionErrorAutomatizacion`, sin llegar a la IA); y una prueba negativa que confirma que aplicar por error la etiqueta `RevisionSinTareas` en vez de `RevisionErrorAutomatizacion` es detectado y rechazado. Resultado de la última ejecución local: **166/166 verificaciones OK** (las 164 anteriores sin cambios + 2 nuevas). Las otras cuatro suites locales del proyecto (`ejecutarPruebasEvaluadorIAFase8()`: 60/60; `ejecutarPruebasExtraerContenidoNuevo()`: 19/19; `ejecutarPruebasPromptObservacionesMixtas()`: 46/46; `ejecutarPruebasSanitizacionHojasTecnicas()`: 17/17) se re-ejecutaron sin cambios de código y no muestran regresiones. Verificación de duplicación (nombres de función de nivel superior y `id` de fixtures): sin duplicados.
