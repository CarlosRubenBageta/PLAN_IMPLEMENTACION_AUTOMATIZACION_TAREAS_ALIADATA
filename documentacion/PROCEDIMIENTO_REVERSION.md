...
8. **Ejecutar una prueba manual de la versión restaurada — con la misma advertencia que B.8 de `PROCEDIMIENTO_DESPLIEGUE.md`:** `script_actual.gs` no tiene modo `DRY_RUN` (auditoría, ALT-14). Ejecutarlo manualmente procesa hasta los hilos reales que encuentre, escribe, etiqueta y archiva — no es una prueba inocua, es ya parte de la recuperación productiva. Por eso el paso 5 (cuarentena) tiene que estar hecho antes de llegar acá. **No dar por sentado que el código restaurado corre limpio solo porque el swap se completó** — el ensayo real del 28/07/2026 (ver "Pendiente") encontró que el script viejo puede fallar contra una planilla que cambió de estructura desde la última vez que corrió ahí. Esta prueba manual es la que detecta eso antes de reactivar el activador, no un paso protocolar.
...
- [x] **Ensayo del swap de código (28/07/2026), sobre el proyecto Apps Script de prueba de la Fase 8** — cubre la parte central del Escenario 3 (BLQ-07 parcialmente cerrado; sigue faltando el simulacro completo con cuarentena y activador real, ver abajo). Reemplazado `script_refactorizado.gs` por `codigo/script_actual.gs`, confirmado que carga y ejecuta; restaurado el código v3 real desde el repo, confirmado con `procesarCorreosDeTareas(): 0 mensajes elegibles, procesando 0` (mismo patrón de log real de toda la Fase 8) que quedó funcionando de nuevo. Dos hallazgos reales:
  1. **El historial de versiones de Apps Script no sirve como red de seguridad** a menos que se hayan guardado versiones explícitas — este proyecto no tenía ninguna, así que no hubo forma de "deshacer" el swap; hubo que reconstruir los 9 archivos pegando manualmente desde el repo. Confirma por qué `PROCEDIMIENTO_DESPLIEGUE.md` (A.1) exige un respaldo externo del proyecto completo antes del corte — no alcanza con confiar en que Apps Script guarda algo internamente.
  2. **El código restaurado no corre garantizado limpio** contra una planilla que cambió de estructura desde la última vez que ese código corrió ahí — el intento inicial con `script_actual.gs` falló (`Sheet ... not found`) en un archivo que había evolucionado desde entonces. Ver la nota agregada en el Escenario 3, paso 8.
- [ ] **Falta el simulacro completo** (BLQ-07): cuarentena de un mensaje parcial simulado, recreación del activador con la cuenta correcta (`tareas@alia-data.com`), y medición de tiempo total de punta a punta — el ensayo de hoy confirmó el mecanismo de restaurar código, pero no esas partes.
- [ ] El resultado del simulacro completo debe anexarse a `auditoria/ACTA_DESPLIEGUE.md` (todavía no existe, ver `PROCEDIMIENTO_DESPLIEGUE.md`).
