...
| `CUENTA_ALERTAS` | `carlosrubenbageta@alia-data.com` | **Confirmado (DEC-017, 28/07/2026):** cuenta técnica externa, uso temporal ("por ahora"). Nunca `tareas@alia-data.com`. **Implementada (30/07/2026, Fase 10):** `codigo/alertas.gs` ya la lee y envía los 7 eventos de alerta que faltaban — ver nota de corrección en DEC-017. Sin esta propiedad configurada, `enviarAlertaTecnica()` solo deja constancia en el log de ejecución y no envía nada (no aborta el pipeline). |
| `COOLDOWN_ALERTAS_MIN` | `60` | **Nueva (30/07/2026, `codigo/alertas.gs`).** Minutos mínimos entre 2 alertas del mismo tipo de evento, para no saturar `CUENTA_ALERTAS` si la misma condición persiste ejecución tras ejecución. Opcional — si falta o es inválida, se usa `60` por defecto; no participa de la validación estricta de `validarConfiguracion()` a propósito (las alertas deben funcionar incluso con la configuración general rota). |
| `UMBRAL_FALLOS_CONSECUTIVOS` | `3` | **Nueva (30/07/2026, `codigo/alertas.gs`).** Ejecuciones consecutivas con al menos un error antes de disparar la alerta "tres fallos consecutivos". Opcional, default `3`. |
| `UMBRAL_REVISION_MANUAL_ALERTA` | `3` | **Nueva (30/07/2026, `codigo/alertas.gs`).** Mensajes a `REVISION_MANUAL` dentro de UNA MISMA ejecución antes de disparar "aumento anormal de revisión manual". Opcional, default `3`. |
