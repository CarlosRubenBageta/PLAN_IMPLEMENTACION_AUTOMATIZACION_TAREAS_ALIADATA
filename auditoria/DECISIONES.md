...
## DEC-017 — Alertas automáticas: notificación nativa de Apps Script ahora, alcance completo diferido a Fase 10

**Fecha:** 2026-07-28  
**Responsable:** Carlos Rubén Bageta (decisión explícita durante la planificación de la Fase 9)  
**Estado:** Aprobada  
**Contexto:** Al planificar el paso "Configurar las alertas hacia la cuenta técnica externa" de la Fase 9, se verificó contra el código real (`codigo/*.gs`) que **no existe ninguna implementación de envío de alertas** — ni `MailApp`, ni `GmailApp.sendEmail()`, ni una función propia. `CUENTA_ALERTAS` está documentada en `configuracion/PARAMETROS_EJEMPLO.md` y los 8 eventos de alerta están listados en la Fase 10 del plan, pero ninguno de los dos está conectado a código que efectivamente mande un email. No es un defecto introducido en esta sesión: no hay registro en `auditoria/CHANGELOG.md` ni `auditoria/DECISIONES.md` de que este subsistema se haya construido, probado o diferido conscientemente en ninguna fase anterior — es una brecha real entre el plan documentado y el código, recién detectada.  
**Decisión:** Para la Fase 9, activar únicamente la notificación nativa de fallas de Apps Script (Configuración del proyecto → notificaciones de fallas de ejecución) apuntando a `carlosrubenbageta@alia-data.com` — sin código nuevo. Cubre solo el evento "runtime terminado inesperadamente" (excepción no controlada). Los otros 7 eventos de la Fase 10 (error crítico de negocio, tres fallos consecutivos, aumento anormal de revisión manual, clave API ausente, falta de permisos, fallo de escritura, hoja inexistente) requieren lógica propia que consulte `Log Mensajes`/`Indice Idempotencia` y decida cuándo alertar — no se construyen ahora.  
**Motivo:** Escribir y probar un subsistema de alertas completo es alcance nuevo, no pasó por ningún caso de prueba de la Fase 8, y arriesgaría introducir código sin validar justo antes del corte productivo. La Fase 10 ya prevé revisión manual diaria (día 1) y dos veces al día (días 2-3) como red de contención durante la primera semana, cubriendo el mismo período en que la ausencia de alertas automáticas más importaría.  
**Impacto:** El criterio de aceptación "Las alertas llegan a la cuenta técnica externa" (Fase 10) **no puede darse por cumplido con esta decisión** más que para el caso de excepción no controlada — queda como brecha explícita a cerrar antes o durante la Fase 10, no asumida como resuelta por la Fase 9.  
**Acciones derivadas:** `CUENTA_ALERTAS=carlosrubenbageta@alia-data.com` (uso temporal — cumple hoy funciones equivalentes a las que tendría `tareas@alia-data.com` en producción, según Carlos Rubén Bageta) registrado en `configuracion/PARAMETROS_EJEMPLO.md`. Pendiente: agregar esta brecha como ítem explícito antes de la puerta de aprobación de la Fase 10 en `PLAN_IMPLEMENTACION_AUTOMATIZACION_TAREAS_ALIADATA_v3.md`.

**Corrección (28/07/2026, auditoría externa de `documentacion/PROCEDIMIENTO_DESPLIEGUE.md`/`PROCEDIMIENTO_REVERSION.md`, hallazgo BLQ-05, verificado independientemente contra documentación oficial de Google — [Installable Triggers](https://developers.google.com/apps-script/guides/triggers/installable)):** la mitigación elegida arriba ("activar la notificación nativa de fallas de Apps Script hacia `carlosrubenbageta@alia-data.com`") **no funciona como se pensó**. Los activadores instalables siempre se ejecutan y notifican bajo la cuenta que los creó; como el activador productivo debe ser creado por `tareas@alia-data.com` (es la única forma de que opere sobre su Gmail), su notificación nativa de fallas queda en esa misma cuenta — exactamente la que este proyecto decidió excluir desde el principio para evitar que una alerta reingrese al flujo. Ser editor del proyecto no alcanza para redirigirla. Esta parte de DEC-017 queda **sin resolver**, no simplemente "cubierta de forma parcial" como decía la versión original. Las tres opciones reales (filtro de reenvío desde `tareas@alia-data.com`, código propio mínimo con `MailApp.sendEmail()`, o postergar todo a la Fase 10) están detalladas en `documentacion/PROCEDIMIENTO_DESPLIEGUE.md`, paso B.11.

**Resolución final (28/07/2026, Carlos Rubén Bageta):** elegida la opción 1 (filtro de reenvío) y **probada de punta a punta con una falla real**, no solo configurada — se forzó una excepción real en un proyecto Apps Script descartable (creado como `tareas@alia-data.com`) y se confirmó que el correo de notificación (`noreply-apps-scripts-notifications@google.com`, asunto "Summary of failures for Google Apps Script: ...") llegó reenviado a `carlosrubenbageta@alia-data.com`. Detalle completo y pasos exactos en `documentacion/PROCEDIMIENTO_DESPLIEGUE.md`, paso B.11. Cubre únicamente "runtime terminado inesperadamente" — los otros 7 eventos de alerta de la Fase 10 siguen sin resolver.
