...
**Archivo:** `PLAN_IMPLEMENTACION_AUTOMATIZACION_TAREAS_ALIADATA_v2.md`  
**Versión:** 2.0 — resultado de la auditoría del 2026-07-19 sobre la versión 1.0  
...
## 0. Cambios de la versión 2.0 respecto de la 1.0

Resultado de la auditoría del plan original:

1. **Búsqueda de mensajes:** se reemplaza la búsqueda por hilos sin etiqueta `Procesado` por una búsqueda `in:inbox` sin filtro de etiqueta. Las etiquetas de Gmail se aplican a nivel de hilo, por lo que el esquema anterior perdía las respuestas nuevas en hilos ya procesados. El descarte de mensajes ya tratados se realiza por idempotencia sobre el ID individual del mensaje (secciones 4.1, 4.8, Fase 3 y Fase 5).
2. **ID determinístico:** se define el método de abreviación del ID de mensaje (hash SHA-256 truncado) para eliminar el riesgo de colisión (Fase 5).
3. **Detección de ambigüedad:** se agregan los campos `requiere_revision` y `motivo_revision` al esquema JSON de la IA, dando mecanismo concreto a la regla "clasificación ambigua → revisión manual" (Fase 2).
4. **Identificadores técnicos sin acentos:** estados, etapas y nombres de archivo se normalizan a ASCII (`REVISION_MANUAL`, `CORREO_EXTRAIDO`, `PROCEDIMIENTO_REVERSION.md`).
5. **Formato de `fecha_limite`:** ISO 8601 (`YYYY-MM-DD`) en la zona horaria del proyecto (Fase 2).
6. **Ubicación del log:** la hoja `Log Procesamiento` se crea en el mismo archivo maestro de Google Sheets, con política de retención (Fase 2).
7. **Límite de cuerpo:** se cuantifica el truncamiento del cuerpo enviado a la IA (parámetro `MAX_CARACTERES_CUERPO`, valor inicial 8.000) (secciones 4.2 y Fase 4).
8. **Cuotas y costos de OpenAI:** HTTP 429 y errores de cuota se tratan como `ERROR_TEMPORAL` con reintento; el consumo se incorpora a los indicadores de monitoreo (Fases 4 y 10).
9. **Verificación de duplicados:** se realiza contra el `Log Procesamiento`, no leyendo las 5 hojas de destino (Fase 5).
10. **Consistencia documental:** la estructura de carpetas incluye todos los `.gs` de las Fases 4–7; se unifica `script_refactorizado.gs`; se define el formato del `Link al correo` (Fase 7).
11. **Fecha de corte:** se agrega un parámetro de fecha de corte para excluir mensajes históricos anteriores al despliegue (secciones 4.8 y 14), y casos de prueba CP-19 y CP-20 (Fase 8).
...
**Regla de búsqueda (v2):** la búsqueda de Gmail será `in:inbox`, **sin filtrar por etiqueta**. El descarte de mensajes ya tratados se realiza comprobando el ID individual de cada mensaje contra el `Log Procesamiento`. Esto garantiza que las respuestas nuevas en hilos ya procesados también generen tareas.
...
**Fecha de corte (v2):** dado que los mensajes procesados por el script anterior no figuran en el nuevo `Log Procesamiento`, se definirá un parámetro `FECHA_CORTE` (fecha del despliegue). Los mensajes con fecha anterior a `FECHA_CORTE` no se procesarán, aunque aparezcan en la bandeja de entrada; se registrarán en el log como excluidos por antigüedad.
...
(descarte de ya procesados y de anteriores a FECHA_CORTE)
    ↓
...
├── PLAN_IMPLEMENTACION_AUTOMATIZACION_TAREAS_ALIADATA_v2.md
├── README.md
├── configuracion/
...
- `fecha_limite` se expresa en formato ISO 8601 (`YYYY-MM-DD`), interpretada en la zona horaria del proyecto.
...
La hoja se crea **dentro del mismo archivo maestro de Google Sheets**.

**Política de retención:** los registros con más de 6 meses de antigüedad se archivarán o purgarán mediante un procedimiento manual documentado en el `MANUAL_OPERATIVO.md`, conservando siempre un respaldo previo.
...
- Usar `GmailApp.search("in:inbox", 0, MAX_HILOS)` **sin filtrar por etiqueta**; el descarte de mensajes ya procesados se realiza por ID de mensaje contra el `Log Procesamiento`.
- Descartar mensajes con fecha anterior a `FECHA_CORTE`.
- Usar `LockService`.
...
- Registrar los tokens consumidos por llamada cuando la API los informe, para estimar costos.
...
```text
ALI-{HASH_MENSAJE}-O{NRO_OBSERVACION}-T{NRO_TAREA}
```

Donde `HASH_MENSAJE` son los primeros 10 caracteres hexadecimales del SHA-256 del ID completo del mensaje de Gmail. No usar truncados directos del ID ni longitudes menores: el hash de 10 caracteres hace despreciable el riesgo de colisión y el método es reproducible.

Ejemplo:

```text
ALI-A8F23C91D0-O02-T01
```

### Reglas

- El mismo mensaje y la misma posición lógica deben generar el mismo ID.
- Antes de escribir una tarea, comprobar si el ID ya existe **contra la hoja `Log Procesamiento` (columna "IDs de tareas")**, no leyendo las 5 hojas de destino, cuya lectura crecería linealmente con el histórico.
...
- Construir el `Link al correo` (columna 13) con el formato `https://mail.google.com/mail/u/0/#all/{ID_DEL_MENSAJE}`, que apunta al mensaje aunque el hilo esté archivado.
...
| CP-17 | Fecha límite explícita | Fecha correcta (ISO 8601) |
| CP-18 | Fecha no explícita | Campo vacío |
| CP-19 | Respuesta nueva en hilo ya procesado | Se procesa y genera tareas |
| CP-20 | Mensaje anterior a `FECHA_CORTE` | Excluido y registrado en el log |
...
FECHA_CORTE: fecha y hora del despliegue
